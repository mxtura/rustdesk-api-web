<template>
  <div class="ab-page">
    ...
    <aside class="ab-rail">
        ...
        <el-dropdown trigger="click">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              ...
              <el-dropdown-item v-if="currentCol" divided @click="deleteCollection">{{ T('Delete') }}</el-dropdown-item>
            </el-dropdown-menu>
          </template>
        </el-dropdown>
      </div>
      ...
      <div class="rail-tags">
        ...
        <div class="rail-tag-list">
          <span class="tag-chip" :class="{active: selectedTags.includes('__none__')}" @click="toggleTag('__none__')">{{ T('NoTag') }}</span>
          <span v-for="t in tagListRes.list" :key="t.id" class="tag-chip" :class="{active: selectedTags.includes(t.name)}" @click="toggleTag(t.name)">
            {{ t.name }}
    ...
    <!-- ОСНОВНОЕ: устройства -->
    <section class="ab-main" v-loading="listRes.loading">
      <div class="ab-toolbar">
        <el-input v-model="search" :placeholder="T('SearchPlaceholder')" :prefix-icon="Search" clearable class="ab-search"/>
...
<script setup>
  ...
  const filtered = computed(() => {
    ...
    if (s) l = l.filter(row => String(row.id).includes(s) || (row.hostname || '').toLowerCase().includes(s) || (row.username || '').toLowerCase().includes(s) || (row.alias || '').toLowerCase().includes(s))
    return l
  })
  ...
  const deleteCollection = async () => {
    ...
    if (res) { ElMessage.success(T('OperationSuccess')); currentCol.value = 0; getCollectionList(); reload() }
  }
  ...
  const deleteTag = async (tag) => {
    ...
    if (res) { ElMessage.success(T('OperationSuccess')); getTagList(); selectedTags.value = selectedTags.value.filter(t => t !== tag.name) }
  }
...
.rt-more { cursor: pointer; }
.rail-tag-list { display: flex; flex-direction: column; gap: 6px; }
.tag-chip {
...
.tag-chip:hover { background: var(--glass-bg-strong); }
.tag-chip.active { background: var(--accent-grad); color: #fff; }
.tag-x { font-size: 12px; opacity: 0; }
.tag-chip:hover .tag-x { opacity: 0.7; }
.tag-x:hover { opacity: 1; }
.rail-foot { display: flex; justify-content: space-between; align-items: center; padding-top: 10px; border-top: 1px solid var(--glass-border); }
.rf-label { font-size: 12px; color: var(--el-text-color-secondary); }

/* основное */
.ab-main { flex: 1; min-width: 0; }
.ab-toolbar { display: flex; gap: 12px; margin-bottom: 16px; }
.ab-search { max-width: 320px; }
...
.dev-card:hover { transform: translateY(-3px); box-shadow: var(--shadow-lift); border-color: var(--glass-border-strong); }
.dev-top {
  height: 96px;
  display: flex; align-items: center; justify-content: center;
  background: linear-gradient(160deg, rgba(0,113,255,0.18), rgba(0,113,255,0.04));
}
.dev-os { width: 44px; height: 44px; }
.dev-name {
  text-align: center;
  font-size: 13px;
...
.dev-id { flex: 1; font-size: 13px; font-variant-numeric: tabular-nums; cursor: pointer; }
.dev-id:hover { color: var(--accent); }
.dev-more { cursor: pointer; }
</style>
