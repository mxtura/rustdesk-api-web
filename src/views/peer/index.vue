  ...
  <div>
    ...
    <el-card class="list-body" shadow="hover">
      <div class="lb-head">
        ...
        <el-button
          v-if="viewMode === 'table'"
          :type="editCols ? 'primary' : ''"
          :icon="Setting"
          @click="editCols = !editCols"
        >
      ...
      <div v-if="viewMode === 'cards'" v-loading="listRes.loading" class="peer-grid">
        <div
          ...
          :key="row.row_id"
          class="peer-card"
          :class="{ online: isOnline(row), selected: isSelected(row) }"
        >
          <el-checkbox
            class="pc-check"
            :model-value="isSelected(row)"
            @change="v => toggleSelect(row, v)"
            @click.stop
          />
          <div class="pc-head">
          ...
          <div class="pc-statusrow">
            <span class="pc-badge" :class="isOnline(row) ? 'on' : 'off'">
              <i class="bd"></i
              >{{
                isOnline(row) ? T('Online') : row.last_online_time ? timeAgo(row.last_online_time * 1000) : T('Offline')
              }}
            </span>
          ...
          <div class="pc-specs">
            <div class="spec" :title="row.cpu">
              <el-icon><Cpu /></el-icon><span>{{ row.cpu || '—' }}</span>
            </div>
            <div class="spec">
              <el-icon><Coin /></el-icon><span>{{ row.memory || '—' }}</span>
            </div>
            <div class="spec" :title="row.os">
              <el-icon><Monitor /></el-icon><span>{{ row.os || '—' }}</span>
            </div>
            <div class="spec">
              <el-icon><Connection /></el-icon><span>{{ row.last_online_ip || '—' }}</span>
            </div>
          </div>
...
<script setup>
  ...
  import { connectByClient, osIcon, isPeerOnline } from '@/utils/peer'
  import {
    ArrowDown,
    ArrowUp,
    CopyDocument,
  ...
  const isOnline = row => isPeerOnline(row.last_online_time)
...
<style scoped lang="scss">
  ...
  .peer-card:not(.online) .pc-osbadge {
    opacity: 0.6;
  }
  .pc-title {
    flex: 1;
