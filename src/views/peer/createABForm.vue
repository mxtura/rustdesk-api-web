<template>
  <el-form ref="form" class="dialog-form" :model="ABFormData" label-width="120px">
    ...
    <el-form-item v-if="ABFormData.user_ids.length <= 1" :label="T('AddressBookName')" required prop="collection_id">
    ...
    <el-form-item :label="T('Platform')" prop="platform">
      <el-select v-model="ABFormData.platform">
        <el-option v-for="item in ABPlatformList" :key="item.value" :label="item.label" :value="item.value"></el-option>
      </el-select>
    </el-form-item>

    <el-form-item v-if="ABFormData.user_ids.length <= 1" :label="T('Tags')" prop="tags">
      <el-select v-model="ABFormData.tags" multiple>
        <el-option v-for="item in tagListRes.list" :key="item.name" :label="item.name" :value="item.name"></el-option>
      </el-select>
    </el-form-item>
    <el-form-item>
