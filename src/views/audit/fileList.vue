<template>
  <div>
      ...
      >
        ...
        <el-table-column prop="type" :label="T('Type')" align="center" width="200">
          <template #default="{ row }">
            <el-tag v-if="row.type === 1" type="warning">
              {{ T('ToRemote') }}:
              <el-icon>
                <Right />
              </el-icon>
        ...
        <el-table-column :label="T('FileInfo')" align="center" width="300">
          <template #default="{ row }">
            <template v-if="!row.is_file">
              ...
              <el-button
                v-if="row.info.files.length > showDirFileNum"
                size="small"
                style="width: 100%; margin-top: 5px"
                type="primary"
                @click="showAllFile(row.info.files)"
              >
    ...
    <el-dialog v-model="allFilesVisible" :title="T('File')">
      ...
      <el-button style="margin-top: 20px; width: 100%" type="primary" @click="allFilesVisible = false">{{
        T('Close')
      }}</el-button>
    </el-dialog>
  </div>
