<template>
  <el-sub-menu
    v-if="
      route.children && route.children.filter(c => !c.meta?.hide).length > 1 && route.children.some(r => !r.meta?.hide)
    "
    :key="route.name"
    :index="route.name"
  >
    ...
    <menu-item v-for="_route in route.children" :key="_route.name" :route="_route"> </menu-item>
  </el-sub-menu>
